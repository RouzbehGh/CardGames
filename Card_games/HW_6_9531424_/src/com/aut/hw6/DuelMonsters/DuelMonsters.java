...
public class DuelMonsters {
    ...
    private static void playerPlayCard() {
        ...
        if (selection >= 0 && selection < player.getCurrent_valid_lengh_of_hand()
                && player.getHand()[selection] != null) {
            success = player.playCardFromHand(selection, playerField);
        } else if (selection == player.getCurrent_valid_lengh_of_hand()
    ...
    private static void playerAttackCard() {
        ...
        if (myMonster >= 0 && enemyMonster >= 0
                && myMonster < playerField.getCurrent_monsters_on_field()
                && enemyMonster < cpuField.getCurrent_monsters_on_field()
                && playerField.getMonsters()[myMonster] != null
                && playerField.getMonsters()[myMonster].isCanAttack()
            ...
            if (success) {
                int temp1 = cpuField.getMonsters()[enemyMonster].getPower() ;
                int temp2 = playerField.getMonsters()[myMonster].getPower() ;
                cpu.changeLifePoints( temp1 - temp2);
                cpuField.reduceMonsterCard(enemyMonster);
    ...
    private static void cpuMove() {
        for (int i = 0; i < cpu.getCurrent_valid_lengh_of_hand(); i++) {
    ...
    private static void printGameState() {
        ...
        System.out.println();
        printNumberedArray(cpuField.getMonsters(),cpuField.getCurrent_monsters_on_field());
        System.out.println("Enemy Spells");
        printNumberedArray(cpuField.getSpells(),cpuField.getCurrent_spells_on_field());
        System.out.println();
        ...
        printNumberedArray(playerField.getMonsters(), playerField.getCurrent_monsters_on_field());
        System.out.println();
        System.out.println("Player Spells");
        printNumberedArray(playerField.getSpells(),playerField.getCurrent_spells_on_field());
        System.out.println();
        System.out.println("Your Hand");
        printNumberedArray(player.getHand(),player.getCurrent_valid_lengh_of_hand());
        System.out.println();
     ...
     */
    private static void printNumberedArray(Object[] things , int count) {
        int i = 1;
        for (int j = 0 ; j < count ; j++) {
            System.out.println(i + ": " + things[j]);
